...
1. **Pixels(px)**: 디스플레이의 가로 및 세로 해상도를 입력합니다.
2. **Ratio/Constant**: 원하는 화면 비율을 설정합니다. mm 값이 존재할 경우 해당 비율로 크기가 확장/축소됩니다.
3. **Millimeters / Inches**: 물리적 크기를 입력합니다. 입력 완료(`Enter`/`Tab`/`Focus Lost`) 시 모든 연관 데이터가 업데이트됩니다.

---

## ■ 제약 사항 및 설정 (Constraints)

● **설계 범위**: 본 프로그램은 아래와 같은 시스템 설정 범위를 참고하여 설계되었습니다.

---

## ■ 설치 및 빌드 (Installation & Build)

1. SFML 라이브러리를 설치하고 프로젝트 환경 변수에 추가합니다. (디버그용과 릴리즈용 라이브러리가 다름에 반드시 주의합니다)
2. `Arial.ttf` 폰트 파일이 시스템 경로에 존재하는지 확인합니다.
3. C++ 컴파일러를 통해 `main.cpp`를 빌드합니다.
